 ...
 */
function getPrevDay(day = 0) {
  let today = new Date();
  let prevDay = new Date();

  if (today.getDay() == day) prevDay.setDate(today.getDate() - 7);
  else prevDay.setDate(today.getDate() - mod(today.getDay() - day, 7));
 ...
 */
function getNextDay(day = 0) {
  let today = new Date();
  let nextDay = new Date();

  if (today.getDay() == day) nextDay.setDate(today.getDate() + 7);
  else nextDay.setDate(today.getDate() + mod(day - today.getDay(), 7));

  return nextDay;
}

/**
 * Convert a date with time format to time sinc epoch of January 1, 1970 UTC.
 *
 * @param {Date} date Date
 * @param {String} t_fmt Time format 'HH:mm:ss', ie '09:30:59
 * @return {Number} Time in milliseconds since epoch
 */
function getTimeSinceEpoch(date = new Date(), t_fmt = '01:00:00') {
